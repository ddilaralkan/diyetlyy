...
// Eye ikonunu buraya dahil ettik.
import {
  ArrowLeft,
  Edit2,
  Trash2,
...
function PatientDetailPage() {
  ...
  async function fetchDietPlans() {
    try {
      if (!id) return;
      const data = await getDietPlansByPatient(id);
      setDietPlans(data.slice(0, 10));
    } catch (error) {
      console.error(error);
    }
  }
  ...
  // Hasta yüklenirken ya da bulunamadığında çıkan ekran
  if (!patient) {
    return (
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Clock3 className="h-5 w-5 text-[#557A2B]" />
                  <h2 className="text-lg font-bold text-gray-900">
                    Son Diyetler
                  </h2>
                </div>
                ...
                <button
                    onClick={() => navigate(`/patients/${patient.id}/diet-plans`)}
                    className="text-sm font-semibold text-[#557A2B] hover:underline"
                >
                  Tümünü Gör
                </button>
              </div>
              ...
              <div className="space-y-3">
                ...
                {dietPlans.map((diet) => (
                    <div
                        key={diet.id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:border-[#557A2B]/30 transition-all duration-200 gap-4 sm:gap-0"
                    >
                      <div>
                        <p className="text-sm font-semibold text-gray-900">
                          {new Date(diet.createdAt).toLocaleString("tr-TR")}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">
                          {diet.dayCount} Günlük {" • "} {diet.title}
                        </p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button
                            onClick={() => navigate(`/diet-plans/${diet.id}`)}
                            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-gray-200 hover:bg-gray-100 text-sm font-medium text-gray-700 transition-colors"
                        >
                        ...
                        <button
                            onClick={() => navigate(`/diet-editor/${diet.id}`)}
                            className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#557A2B] text-white hover:opacity-90 text-sm font-medium transition-opacity"
                        >
                          <Edit2 className="h-4 w-4" />
                          Düzenle
                        </button>
